...
export function AdminDashboard({ user }: AdminDashboardProps) {
  ...
  const [stats, setStats] = useState({
    ...
    totalCourses: 0,
    activeCourses: 0,
    recentActivities: [] as { type: string, action: string, timeFormatted: string }[],
  });
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div className="text-2xl font-semibold text-gray-900 mb-1">NextGenLMS</div>
            <div className="text-sm text-gray-600">Xin chào</div>
            <div className="flex items-center gap-2 mt-2">
              <div className="text-xs text-success-600 font-medium">System admin</div>
            </div>
          </div>
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              stats.recentActivities.map((activity, index) => (
                <div key={index} className="flex items-start gap-4 p-4 rounded-lg hover:bg-gray-50">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-500 mt-1">{activity.timeFormatted}</p>
                  </div>
                </div>
              ))}
          </div>
