...
export function AssessmentListPage({ user }: AssessmentListPageProps) {
  ...
  const renderAssessmentCard = (assessment: typeof mockAssessments[0]) => {
    ...
    return (
      <div
        key={assessment.id}
        className={`p-4 border rounded-lg hover:shadow-md transition-all ${overdueStatus ? 'border-danger-300 bg-danger-50' : 'border-gray-200 bg-white'
          }`}
      >
        ...
        <div className="flex gap-2">
          {!submission?.submittedAt ? (
            <button
              ...
              className={`px-4 py-2 rounded-lg font-medium transition-colors ${overdueStatus
